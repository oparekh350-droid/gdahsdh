import type { UserRole, BusinessType } from "@/shared/types"
...
class AuthService {
  ...
  async findBusinessByName(businessName: string): Promise<Business | null> {
    const { businessRepository } = await import("@/services/indexeddb/repositories/businessRepository")
    const found = await businessRepository.findByName(businessName)
    if (found) {
      return {
        id: found.id,
        name: found.name,
        type: found.type as BusinessType,
        ownerId: "",
        ownerEmail: found.ownerEmail,
        ownerPasswordHash: found.ownerPasswordHash || "",
        staffPasswordHash: found.staffPasswordHash || "",
        businessCode: found.businessCode,
        plan: found.plan,
        createdAt: found.createdAt,
        settings: { currency: "INR", timezone: "Asia/Kolkata", language: "en" },
      }
    }
  ...
  async ownerSignup(signupData: OwnerSignupData): Promise<{
    success: boolean
    user?: AuthUser
    tokens?: AuthTokens
    message: string
    ...
    try {
      ...
      const allowedBusinessTypes: BusinessType[] = ["manufacturer", "retailer"]
      if (!allowedBusinessTypes.includes(signupData.businessType)) {
        return {
          success: false,
          message: "Selected business type is not supported",
        }
      }
      ...
      if (ownerPasswordError) {
        return {
          success: false,
          message: `Owner password: ${ownerPasswordError}`,
        }
      }
      ...
      if (staffPasswordError) {
        return {
          success: false,
          message: `Staff password: ${staffPasswordError}`,
        }
      }
      ...
      if (signupData.ownerPassword === signupData.staffPassword) {
        return {
          success: false,
          message: "Owner and staff passwords must be different",
        }
      }

      // Check for duplicate business name
      if (await this.findBusinessByName(signupData.businessName)) {
        return {
          success: false,
      ...
      const { businessRepository } = await import("@/services/indexeddb/repositories/businessRepository")
      const ownerPasswordHash = await this.hashPassword(signupData.ownerPassword)
      const staffPasswordHash = await this.hashPassword(signupData.staffPassword)
      ...
      // Store in databases
      this.businessDatabase.set(business.id, business)
      ...
      return {
        ...
        user,
        tokens,
        message: "Business created and owner logged in successfully",
      }
    } catch (error) {
      return {
        success: false,
  ...
  async staffSignin(signinData: StaffSigninData): Promise<{
    success: boolean
    user?: AuthUser
    tokens?: AuthTokens
    message: string
    ...
    try {
      ...
      if (signinData.staffPassword.length < 8 || signinData.staffPassword.length > 50) {
        return {
          success: false,
          message: "Staff password must be between 8-50 characters",
        }
      }
      ...
      if (restrictedRoles.includes(signinData.role)) {
        return {
          success: false,
          message:
            "Owner and Co-Founder accounts cannot be created through staff sign-in. Use business owner sign-up instead.",
        }
      }

      // Find business by name
      const business = await this.findBusinessByName(signinData.businessName)
      if (!business) {
        return {
          success: false,
          message: "Business not found. Please check the business name or contact your business owner.",
        }
      }

      // Verify staff password
      if (!(await this.verifyPassword(signinData.staffPassword, business.staffPasswordHash))) {
        return {
          success: false,
          message: "Invalid staff password. Please contact your business owner for the correct password.",
        }
      }
      ...
      for (const user of this.userDatabase.values()) {
        if (user.businessId === business.id && user.email.toLowerCase() === signinData.email.toLowerCase()) {
          return {
            success: false,
            message:
              "An account with this email already exists in this business. Please use a different email or contact your administrator.",
          }
        }
      }
      ...
      for (const user of this.userDatabase.values()) {
        if (user.businessId === business.id && user.phone === signinData.phone) {
          return {
            success: false,
            message:
              "An account with this phone number already exists in this business. Please use a different phone number.",
          }
        }
      }
      ...
      if (!allowedRoles.includes(signinData.role)) {
        return {
          success: false,
          message: "Invalid role selection. Please choose a valid staff role.",
        }
      }
      ...
      if (signinData.email.toLowerCase() === business.ownerEmail.toLowerCase()) {
        return {
          success: false,
          message: "This email is reserved for the business owner. Staff members must use a different email address.",
        }
      }
      ...
      if (user.isOwner) {
        return {
          success: false,
          message: "Security error: Invalid role assignment detected.",
        }
      }
      ...
      return {
        success: false,
        message: "Sign in failed. Please check your information and try again.",
      }
    }
  }
  ...
  getPermissionsForRole(role: UserRole): string[] {
    const basePermissions = ["view_dashboard", "view_basic_analytics", "view_inventory", "view_orders"]
  ...
  async verifyBusinessStaffPassword(businessName: string, password: string): Promise<boolean> {
    const biz = await this.findBusinessByName(businessName)
    if (!biz) return false
    return this.verifyPassword(password, biz.staffPasswordHash)
  }

  private async findBusinessByCode(code: string): Promise<Business | null> {
    const { businessRepository } = await import("@/services/indexeddb/repositories/businessRepository")
    const found = await businessRepository.findByCode(code)
  ...
  async verifyBusinessStaffPasswordByCode(code: string, password: string): Promise<boolean> {
    const biz = await this.findBusinessByCode(code)
    if (!biz) return false
    return this.verifyPassword(password, biz.staffPasswordHash)
  }
  ...
  async createBusinessProfile(input: {
    ...
    if (!input.businessName.trim() || !input.ownerName.trim()) {
      return { success: false, message: "Business and owner name are required" }
    }
    if (!this.validateEmail(input.email)) return { success: false, message: "Invalid email format" }
    if (!this.validatePhone(input.phone)) return { success: false, message: "Invalid phone number format" }
    if (await this.findBusinessByName(input.businessName)) {
    ...
    const business: Business = {
      id: created.id,
      name: created.name,
      type: created.type as BusinessType,
      ownerId: "",
      ownerEmail: created.ownerEmail,
      ownerPasswordHash: created.ownerPasswordHash || "",
      staffPasswordHash: created.staffPasswordHash || "",
      businessCode: created.businessCode,
      plan: created.plan,
      createdAt: created.createdAt,
      settings: { currency: "INR", timezone: "Asia/Kolkata", language: "en" },
    }
    // Save latest business in local storage for profile code display
    localStorage.setItem(this.BUSINESS_DATA_KEY, JSON.stringify(business))
    this.businessDatabase.set(business.id, business)
    return { success: true, business }
  }

  async finalizeBusinessSecurity(
    businessId: string,
    ownerPassword: string,
    staffPassword: string,
  ): Promise<{ success: boolean; user?: AuthUser; tokens?: AuthTokens; message: string }> {
    try {
      const { businessRepository } = await import("@/services/indexeddb/repositories/businessRepository")
      const bizStored = await businessRepository.findById(businessId)
      if (!bizStored) return { success: false, message: "Business not found" }

      const ownerPasswordHash = await this.hashPassword(ownerPassword)
      const staffPasswordHash = await this.hashPassword(staffPassword)
      const updated = { ...bizStored, ownerPasswordHash, staffPasswordHash }
      ...
      const user: AuthUser = {
        ...
        businessType: updated.type as BusinessType,
        permissions: this.getPermissionsForRole("owner"),
        isOwner: true,
      }
      ...
      const business: Business = {
        id: updated.id,
        name: updated.name,
        type: updated.type as BusinessType,
        ownerId: userId,
      ...
      // Tokens
      const accessToken = this.generateJWT(user, 24 * 60 * 60 * 1000)
      const refreshToken = this.generateJWT({ userId: user.id }, 7 * 24 * 60 * 60 * 1000)
      const tokens: AuthTokens = { accessToken, refreshToken, expiresAt: Date.now() + 24 * 60 * 60 * 1000 }
      ...
      notificationService.success(
        ...
        `Your Business Code is ${business.businessCode}. Share it with staff to join.`,
      )
      return { success: true, user, tokens, message: "Owner set passwords successfully" }
    } catch {
      return { success: false, message: "Failed to finalize business security" }
    }
  }
}
