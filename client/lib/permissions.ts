...
export const initializePermissions = (user: any, business: any) => {
  return {
    getAllPermissions: () => {
      ...
      const rolePermissions = authService.hasRole(user.role) ? authService.getCurrentUser()?.permissions || [] : []
  ...
  return {
    ...
    hasModuleAccess: (moduleId: string): boolean => {
      ...
      const roleModuleAccess: Record<UserRole, string[]> = {
        ...
        staff: ["orders", "customers", "basic-inventory", "pos", "basic-reports"],
        accountant: ["analytics", "financial-reports", "expenses", "gst-reports", "profit-loss", "ai-assistant"],
        sales_executive: ["customers", "orders", "sales", "commissions", "targets", "ai-assistant"],
      }
