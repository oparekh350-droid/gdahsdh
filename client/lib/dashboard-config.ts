...
export const KPI_CONFIGS: KPIConfig[] = [
  ...
  },
  {
    ...
    businessTypes: ["retailer", "ecommerce", "manufacturer", "distributor", "trader"],
  },
  {
  ...
  },
  {
    ...
    category: "operations",
    permissions: ["view_orders"],
    businessTypes: ["ecommerce", "manufacturer"],
  },
  ...
  },
  {
    ...
    businessTypes: ["distributor", "sales_executive"],
  },
...
export function generateDashboardConfig(
  ...
  // Categorize widgets
  const primaryWidgets = widgets.filter((w) => w.category === "primary" && w.priority <= 4).slice(0, 6)

  const secondaryWidgets = widgets
    .filter((w) => w.category === "secondary" || (w.category === "primary" && w.priority > 4))
    .slice(0, 8)

  const analyticsWidgets = widgets.filter((w) => w.category === "analytics").slice(0, 4)

  const settingsWidgets = widgets.filter((w) => w.category === "settings").slice(0, 4)
...
function generateQuickActions(
  ...
  return actions.filter((action) => action.permissions.some((permission) => userPermissions.includes(permission)))
}
...
export const DASHBOARD_MODULES = {
  ...
  manufacturer: [
    ...
    },
    {
      ...
      icon: "FileText",
      path: "/dashboard/manufacturer/bill-of-materials",
    },
    {
      id: "cost-calculation",
