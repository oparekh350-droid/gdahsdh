...
import { PieChart, Eye, EyeOff, ArrowLeft, Loader2, User, Users, Shield, CheckCircle, XCircle } from "lucide-react"
import { authService, type StaffSigninData } from "@/lib/auth-service"
...
const Signin: React.FC = () => {
  ...
  const validateField = (field: keyof StaffSigninData, value: string): string => {
    switch (field) {
      case "businessCode":
        if (!value.trim()) return "Business code is required"
        if (!/^[A-Z0-9]{8}$/.test(value.trim().toUpperCase())) return "Enter 8-character code (A-Z, 0-9)"
        return ""
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const ok = await authService.verifyBusinessStaffPasswordByCode(
        trimmedData.businessCode,
        trimmedData.staffPassword,
      )
      if (!ok) {
        setError("Invalid business code or password. Please contact your business owner.")
        return
      }

      const { businessRepository } = await import("@/services/indexeddb/repositories/businessRepository")
      const biz = await businessRepository.findByCode(trimmedData.businessCode)
      if (!biz) {
        setError("Business not found for the provided code")
        return
      }

      const { staffRequestRepository } = await import("@/services/indexeddb/repositories/staffRequestRepository")
      const activeStaffCount = await staffRequestRepository.getActiveCountByBusinessCode(trimmedData.businessCode)
      const staffLimit = biz.plan === "PREMIUM" ? 30 : 3

      if (activeStaffCount >= staffLimit) {
        setError(
          `Staff limit reached for ${biz.plan} plan (${staffLimit} max). Contact owner to upgrade or remove inactive staff.`,
        )
        return
      }

      await staffRequestRepository.add({
        businessName: biz.name,
        businessCode: biz.businessCode,
        businessId: biz.id,
        staffName: trimmedData.name,
      ...
      const { notificationsApi } = await import("@/lib/notifications-api")
      await notificationsApi.create({
        type: "info" as any,
        title: "New Staff Access Request",
        message: `${trimmedData.name} (${trimmedData.role}) requested access to join ${biz.name}`,
      ...
      navigate("/", {
        replace: true,
        state: {
          message: `Access request sent to ${biz.name} owner for approval. You'll be notified once approved.`,
          type: "success",
        },
      })
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Connect to your business account as a team member</p>
        </div>
        ...
        <Card className="shadow-xl border-0">
          <CardHeader className="space-y-1 pb-6">
            ...
            <CardDescription className="text-center">Request access to join your business team</CardDescription>
          </CardHeader>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <Alert className="border-blue-200 bg-blue-50">
                ...
                <AlertDescription className="text-blue-700">
                  <strong>How it works:</strong> Enter your Business Profile Code and staff password. Your request will
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="businessCode">Business Profile Code *</Label>
                  <div className="relative">
                    <Input
                      ...
                      className={
                        validationErrors.businessCode
                          ? "border-red-500 pr-10"
                          : fieldValidation.businessCode
                    ...
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                      {getFieldIcon("businessCode")}
                    </div>
                  </div>
                  <p className="text-xs text-gray-500">
                    Ask your business owner for the 8-character Business Profile Code
                  </p>
                  {validationErrors.businessCode && (
                    <p className="text-sm text-red-500">{validationErrors.businessCode}</p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">Provided by your business owner</p>
                  {validationErrors.staffPassword && (
              ...
              <div className="space-y-4 pt-4 border-t">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      maxLength={20}
                    />
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2">{getFieldIcon("phone")}</div>
                  </div>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={formData.role} onValueChange={(value) => handleInputChange("role", value as UserRole)}>
                    <SelectTrigger
                      className={
                        validationErrors.role ? "border-red-500" : fieldValidation.role ? "border-green-500" : ""
                      }
                    >
                      <SelectValue />
                    </SelectTrigger>
                  ...
                  <p className="text-xs text-gray-500">Select the role that best matches your responsibilities</p>
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    ...
                    Request Access as {availableRoles.find((r) => r.value === formData.role)?.label}
                  </>
                )}
              </Button>
